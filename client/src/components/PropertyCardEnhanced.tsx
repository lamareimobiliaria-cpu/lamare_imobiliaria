...
import { Heart, MapPin, BedDouble, Bath, Car, Waves, TrendingUp, Zap } from "lucide-react";
...
export default function PropertyCardEnhanced({ property, showAnalysis = true }: PropertyCardEnhancedProps) {
  ...
  const { data: analysis } = trpc.properties.analyze.useQuery(
    { propertyId: property.id },
  ...
  return (
    <Link href={`/properties/${property.id}`}>
      <div className="group bg-card rounded-2xl overflow-hidden border border-border hover:border-accent/40 hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col h-full">
        {/* Image */}
        <div className="relative h-52 overflow-hidden bg-muted">
          ...
          {property.beachDistance && (
            <div className="absolute bottom-3 left-3 flex items-center gap-1 bg-black/50 text-white text-xs px-2 py-1 rounded-full backdrop-blur-sm">
              ...
              <span>{property.beachDistance < 1000 ? `${property.beachDistance}m` : `${(property.beachDistance / 1000).toFixed(1)}km`}</span>
            </div>
          )}
        </div>
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          {showAnalysis && analysis && (
            <div className="mb-4 p-3 bg-accent/5 rounded-lg border border-accent/20 space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground flex items-center gap-1">
                  <Zap className="w-3 h-3 text-accent" /> ROI Estimado
                </span>
                <span className="font-semibold text-green-600">{analysis.roi}% a.a.</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">Valorização</span>
                <span className="font-semibold text-blue-600">{analysis.estimatedAppreciation}% a.a.</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">Aluguel Est.</span>
                <span className="font-semibold text-accent">R$ {(analysis.estimatedMonthlyRent ?? 0).toLocaleString("pt-BR")}</span>
              </div>
            </div>
          )}
